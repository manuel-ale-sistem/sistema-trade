...
def consultas() -> None:
  ...
  if buscar:
    mascara = df["folio"].astype(str).str.contains(buscar, case=False, na=False)
    df = df[mascara]
  ...
  st.download_button(
      label="📥 Exportar Excel", data=excel, file_name="consulta_trade.xlsx"
  )
  ...
  folio = st.selectbox(
      "Seleccione Folio", sorted(df["folio"].dropna().unique().tolist())
  )
  ...
  try:
    response_req = (
        supabase.table("solicitud_detalle")
        .select("*")
        .eq("folio", folio)
  ...
  if detalle_req.empty:
    ...
    for _, req in detalle_req.iterrows():
      with st.container(border=True):
        st.markdown(f"### {req['tipo_solicitud']}")

        st.write(f"**Categoría:** {req['categoria']}")

        if pd.notna(req.get("modelo")) and req.get("modelo") != "":
          st.write(f"**Modelo:** {req['modelo']}")

        if pd.notna(req.get("cantidad")):
          st.write(f"**Cantidad:** {req['cantidad']}")

        if pd.notna(req.get("serie")) and req.get("serie") != "":
          st.write(f"**Serie:** {req['serie']}")

        if pd.notna(req.get("comentarios")) and req.get("comentarios") != "":
          st.info(req["comentarios"])

    # Resumen en tabla y métrica
    st.write("### 📋 Resumen de Requerimientos")
    st.dataframe(detalle_req, use_container_width=True)
    st.metric("Total Requerimientos", len(detalle_req))
  ...
  try:
    response_docs = (
        supabase.table("documentos").select("*").eq("folio", folio).execute()
    )
  ...
  if docs.empty:
    ...
    for _, doc in docs.iterrows():
      ...
      if url_archivo:
        st.link_button(f"📎 {nombre_archivo}", url_archivo)
      else:
        st.warning(
            f"La evidencia {nombre_archivo} no cuenta con una URL válida."
        )
